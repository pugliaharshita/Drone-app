...
import { Plane, Clipboard, User as UserIcon, CheckCircle, LayoutDashboard, Download, Clock, X } from 'lucide-react';
            ...
            <div className="space-y-8">
              <div className="flex justify-between items-center">
                ...
                <button
                  ...
                  className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-xl font-medium shadow-lg shadow-blue-600/10 transition-all hover:shadow-xl hover:shadow-blue-600/20 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
                >
                  Register New Drone
          </button>
        </div>
              ...
              {registeredDrones.length === 0 ? (
                <div className="text-center py-16 bg-white rounded-2xl shadow-xl border border-slate-100">
                  ...
                  <p className="text-slate-600 text-lg mb-4">You haven't registered any drones yet.</p>
                  <button
                    onClick={() => setStep('owner')}
                ...
                <div className="grid gap-6">
                  {registeredDrones.map((drone) => (
                    <div 
                      key={drone.id} 
                      className="bg-white p-6 rounded-2xl shadow-xl border border-slate-100 hover:shadow-2xl transition-shadow cursor-pointer"
                      onClick={() => {
                    ...
                    >
                      ...
                      <div className="mt-4 flex items-center justify-between text-sm text-slate-600">
                        <p>Weight: {drone.weight} grams</p>
                        <p className="text-slate-400">
                          Registered on: {new Date(drone.created_at).toLocaleDateString()}
                        </p>
                      </div>
                    </div>
                  ))}
...
Purpose: ${selectedDrone.purpose}`}
                        ...
                        <div className="px-4 py-2 bg-blue-50 rounded-lg border border-blue-100">
                          <p className="text-sm text-blue-700">Print this QR code and attach it to your drone for easy identification</p>
                        </div>
                      </div>
                    )}
          ...
          {step !== 'dashboard' && (
            <>
              <div className="flex justify-between mb-12 relative">
                ...
                <div className={`relative z-10 flex flex-col items-center ${step === 'owner' ? 'text-blue-600' : 'text-slate-400'}`}>
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center mb-2 ${step === 'owner' ? 'bg-blue-600 text-white' : 'bg-slate-100'}`}>
                    <UserIcon className="h-5 w-5" />
                  </div>
                  <span className="text-sm font-medium">Owner</span>
                </div>
                <div className={`relative z-10 flex flex-col items-center ${step === 'drone' ? 'text-blue-600' : 'text-slate-400'}`}>
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center mb-2 ${step === 'drone' ? 'bg-blue-600 text-white' : 'bg-slate-100'}`}>
                    <Plane className="h-5 w-5" />
                  </div>
                  <span className="text-sm font-medium">Drone</span>
                </div>
                <div className={`relative z-10 flex flex-col items-center ${step === 'review' ? 'text-blue-600' : 'text-slate-400'}`}>
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center mb-2 ${step === 'review' ? 'bg-blue-600 text-white' : 'bg-slate-100'}`}>
                    <Clipboard className="h-5 w-5" />
                  </div>
                  <span className="text-sm font-medium">Review</span>
                </div>
                <div className={`relative z-10 flex flex-col items-center ${step === 'success' ? 'text-blue-600' : 'text-slate-400'}`}>
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center mb-2 ${step === 'success' ? 'bg-blue-600 text-white' : 'bg-slate-100'}`}>
                ...
                <form onSubmit={handleOwnerSubmit} className="bg-white/70 backdrop-blur-lg rounded-2xl shadow-xl border border-slate-100 p-8">
                  ...
                  <div className="flex justify-end mt-8">
                    <button
                      ...
                      className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-xl font-medium shadow-lg shadow-blue-600/10 transition-all hover:shadow-xl hover:shadow-blue-600/20 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
                    >
                      {hasOwnerProfile ? 'Update Profile' : 'Register Profile'}
                    </button>
                  </div>
                </form>
              )}
              ...
              {step === 'drone' && (
                <form onSubmit={handleDroneSubmit} className="bg-white/70 backdrop-blur-lg rounded-2xl shadow-xl border border-slate-100 p-8">
                  <h2 className="text-2xl font-semibold text-slate-900 mb-6">Drone Information</h2>
                  <div className="space-y-6">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-slate-700 mb-1">Weight (in grams)</label>
                      <input
                        type="number"
                        required
                    ...
                    <div>
                      <label className="block text-sm font-medium text-slate-700 mb-1">Purpose of Use</label>
                      <select
                        required
